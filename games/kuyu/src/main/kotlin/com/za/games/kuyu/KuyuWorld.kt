 ...
 * Aynı tohum + aynı girdi dizisi = aynı oyun; günlük mod buna dayanır.
 *
...
class KuyuWorld(
    ...
    /** Bir kare ilerletir; oyun bittiyse hiçbir şey yapmaz. */
    ...
    private fun updateBullets() {
        ...
        while (it.hasNext()) {
            ...
            if (hit || b.traveled >= BULLET_RANGE) it.remove()
        }
    }
    ...
    private fun damage(e: Enemy, amount: Int, stomp: Boolean) {
        ...
        if (e.hp > 0) return
        e.alive = false
        spawnGems(e.centerX, e.centerY, e.kind.gems)
        if (!player.grounded) {
            combo++
