 ...
 * kalmasını istiyor ve HUD sayıları zaten öyle ([ZaLocale.number]).
 ...
 */
fun zaText(resources: Resources, @StringRes id: Int, vararg args: Any): String =
    String.format(Locale.ROOT, resources.getString(id), *args)

/** Bağlamdan okuyan kolaylık; kaynakları bağlamdan alır. */
fun zaText(context: Context, @StringRes id: Int, vararg args: Any): String =
    zaText(context.resources, id, *args)

/** [stringResource]'un Latin rakam garantili karşılığı. */
@Composable
@ReadOnlyComposable
fun zaString(@StringRes id: Int, vararg args: Any): String {
    // stringResource de böyle yapar: yapılandırma değişince metin yenilensin.
    LocalConfiguration.current
    return zaText(LocalContext.current.resources, id, *args)
}
