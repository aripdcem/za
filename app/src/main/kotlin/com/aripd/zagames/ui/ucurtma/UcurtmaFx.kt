...
import com.aripd.zagames.platform.Sfx
...
import com.aripd.zagames.ucurtma.UcurtmaEvent
...
class UcurtmaFx {
    ...
    fun update(dt: Float, world: UcurtmaWorld) {
        val scroll = if (world.status == com.aripd.zagames.ucurtma.UcurtmaStatus.RUNNING) world.speed else 0f
        for (c in clouds) {
            c.x -= scroll * 0.15f * dt
            if (c.x < -0.3f) {
