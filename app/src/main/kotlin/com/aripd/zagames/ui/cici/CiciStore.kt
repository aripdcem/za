package com.aripd.zagames.ui.cici

import android.content.Context

/** Günün uzayı: kullanılan deneme sayısı ve en iyi skor. */
data class CiciDaily(val epochDay: Long, val attempts: Int, val best: Int)

/** Cici kalıcı durumu: günün denemeleri ve en iyisi, serbest rekor, son mod. Yalnızca cihazda. */
class CiciStore(context: Context) {

    private val prefs =
        context.applicationContext.getSharedPreferences("za_cici", Context.MODE_PRIVATE)

    fun daily(epochDay: Long): CiciDaily? {
        if (prefs.getLong(KEY_DAY, Long.MIN_VALUE) != epochDay) return null
        return CiciDaily(epochDay, prefs.getInt(KEY_ATTEMPTS, 0), prefs.getInt(KEY_DAY_BEST, 0))
    }
